...
if(isset($_POST["submit"])){
    ...
    $res = mysqli_query($con,"SELECT * FROM `$table` WHERE `coupon_code`='$coupon'");
    if(mysqli_num_rows($res)>0){
        if(isset($_GET["id"]) && $_GET["id"]!=""){
    ...
    if(isset($_GET["id"]) && $_GET["id"]!=""){
        mysqli_query($con,"UPDATE `$table` SET `coupon_code`='$coupon', `price`='$price' WHERE `id`='$id'");
        $_SESSION["msg"]='<div class="alert alert-success msg" role="alert">
        <strong>Success!</strong> Updated.</div>';
      }else{
          //IF URL NOT HAVE THE ID THEN INSERT DATA
        mysqli_query($con,"INSERT INTO `$table` (`coupon_code`,`price`,`status`) VALUES ('$coupon','$price','1')");
        $_SESSION["msg"]='<div class="alert alert-success msg" role="alert">
                ...
                <div class="col-12 px-0">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h3 class="card-title text-uppercase font-weight-bold"><?php if(isset($_GET["id"]) && $_GET["id"]!=""){echo $update;}else{echo $add;}?> coupon</h3>
                        </div>
                        <div class="card-body">
                            <form action="" method="post" class="row">
                                <div class="col-md-6 p-3">
                                    <input type="text" name="coupon" value="<?php echo $coupon;?>" class="form-control" placeholder="E.g, Blessed21, OFF20 " required autofocus>
                                </div>
                                <div class="col-md-6 p-3">
                                    <input type="text" name="price" value="<?php echo $price;?>" class="form-control" placeholder="E.g 10, 20, 30" required>
                                </div>
                                <div class="col-12 p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-md-2 px-2"> <a href="./coupon" class="btn btn-info btn-block btn-lg text-uppercase">View</a>
                                            </div>
                                        </div>
                                    </div>
                            </form>
