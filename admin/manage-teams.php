...
if (isset($_POST["submit"])) {
    if (empty($_FILES["image"]["name"])) {
        ...
        unlink('../assets/media/teams/' . $image);
        $image = $_FILES["image"]["name"];
        $image = rand(111111111, 999999999) . "-" . $image;
        $image = str_replace(" ", "-", $image);
        move_uploaded_file($_FILES["image"]["tmp_name"], '../assets/media/teams/' . $image);
    }
    //FOR UPDATE DATA FETCH FROM THE DB TO SHOW BEFORE EDIT END
    //IF URL HAVE THE ID FOR UPDATE DATA
    if (isset($_GET["id"]) && $_GET["id"] != "") {
        unlink('../assets/media/teams/' . $image);
        mysqli_query($con, "UPDATE `teams` SET `image`='$image' WHERE `id`='$id'");
        ...
        mysqli_query($con, "INSERT INTO `teams` (`image`,`status`) VALUES ('$image','1')");
        $_SESSION["msg"] = '<div class="alert alert-success msg" role="alert">
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="row">
                                    <div class="col-md-12 p-3">
                                        <label for="image">teams Image</label>
                                ...
                                <div class="col-12 p-3">
                                    <div class="row">
                                        ...
                                        <div class="col-md-2 px-2"> <a href="./teams" class="btn btn-info btn-block btn-lg text-uppercase">View</a>
                                        </div>
                                    </div>
                                </div>
                            </form>
