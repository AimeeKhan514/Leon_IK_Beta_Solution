...
if ( isset( $_POST["submit"] ) ) {
    ...
    $res = mysqli_query( $con, "SELECT * FROM `$table` WHERE `categories`='$categories'" );
    if ( mysqli_num_rows( $res )>0 ) {
        if ( isset( $_GET["id"] ) && $_GET["id"] != "" ) {
    ...
    if ( $msg == "" ) {
        ...
        if ( isset( $_GET["id"] ) && $_GET["id"] != "" ) {
            mysqli_query( $con, "UPDATE `$table` SET `categories`='$categories' WHERE `id`='$id'" );
            $_SESSION["msg"] = '<div class="alert alert-success msg" role="alert">
                <strong>Success!</strong> Updated.</div>';
        } else {
            //IF URL NOT HAVE THE ID THEN INSERT DATA
            mysqli_query( $con, "INSERT INTO `$table` (`categories`,`status`) VALUES ('$categories','1')" );
            $_SESSION["msg"] = '<div class="alert alert-success msg" role="alert">
...
<input type = "text" name = "categories" value = "<?php echo $categories;?>" class = "form-control" placeholder = "Category Title" required autofocus >
</div>
<div class="col-12 p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-md-2 px-2"> <a href="./categories" class="btn btn-info btn-block btn-lg text-uppercase">View</a>
                                            </div>
                                        </div>
                                    </div>
</form>
