...
if(isset($_POST["submit"])){
    $floating_news = get_safe_value($con,$_POST["floating_news"]);
    //FOR UPDATE DATA FETCH FROM THE DB TO SHOW BEFORE EDIT
    $res = mysqli_query($con,"SELECT * FROM `$table` WHERE `floating_news`='$floating_news'");
    if(mysqli_num_rows($res)>0){
        if(isset($_GET["id"]) && $_GET["id"]!=""){
    ...
    if(isset($_GET["id"]) && $_GET["id"]!=""){
        mysqli_query($con,"UPDATE `$table` SET `floating_news`='$floating_news' WHERE `id`='$id'");
        $_SESSION["msg"]='<div class="alert alert-success msg" role="alert">
        <strong>Success!</strong> Updated.</div>';
      }else{
          //IF URL NOT HAVE THE ID THEN INSERT DATA
        mysqli_query($con,"INSERT INTO `$table` (`floating_news`,`status`) VALUES ('$floating_news','1')");
        $_SESSION["msg"]='<div class="alert alert-success msg" role="alert">
                ...
                <div class="col-12 px-0">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h3 class="card-title text-uppercase font-weight-bold"><?php if(isset($_GET["id"]) && $_GET["id"]!=""){echo $update;}else{echo $add;}?> floating news</h3>
                        </div>
                        <div class="card-body">
                            <form action="" method="post">
                                <div class="col-md-6 p-3">
                                    <input type="text" name="floating_news" value="<?php echo $floating_news;?>" class="form-control" placeholder="Floating News" required autofocus>
                                </div>
                                <div class="col-12 p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-md-2 px-2"> <a href="./floating-news" class="btn btn-info btn-block btn-lg text-uppercase">View</a>
                                            </div>
                                        </div>
                                    </div>
                            </form>
