...
<h3 class = "card-title float-left text-uppercase font-weight-bold">Categories
</h3>
<a href = "manage-categories" class = "card-title float-right btn btn-primary">Add
Categories
</a>
</div>
...
?>
        ...
        <td><?php echo $row["id"];
        ?></td>
        <td title="<?php echo $row["categories"]; ?>" rel="tooltip"><?php echo $row["categories"];
        ?></td>
        ...
        ?></td>
        <td>
        <a href = "manage-categories?id=<?php echo $row['id'] ?>"
       ...
       <?php if (isset($_SESSION["ADMIN_ROLE"]) && $_SESSION["ADMIN_ROLE"] == "1") {
                    ?>
        <a href = "?type=delete&id=<?php echo $row['id'] ?>" id="btn_delete" 
